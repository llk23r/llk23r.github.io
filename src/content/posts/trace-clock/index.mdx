...
<FrequencyDivider client:only="svelte" />
...
<CrystalDrift client:only="svelte" />
...
<TimeAsState client:only="svelte" />
...
<NTPHandshake client:only="svelte" />
...
<StratumHierarchy client:only="svelte" />
...
<CesiumFeedbackLoop client:only="svelte" />
...
<IonosphereReflection client:only="svelte" />
...
<RelativisticDrift client:only="svelte" />
...
<SystemDiagram client:only="svelte" />
...
Cesium has defined the second since 1967. Before that, the Earth's rotation did; one day divided into 86,400 equal parts. Planets kept time for centuries. Then Essen and Parry showed that atoms keep better time than planets, and the definition changed.

Now cesium itself is being surpassed.

In Boulder, Colorado, a few miles from NIST-F2, the clocks that will retire it are already running. They trap strontium or ytterbium atoms in a grid of laser light and measure their energy transitions; the same principle as the cesium clock, but at visible-light frequencies instead of microwaves. Hundreds of trillions of cycles per second, compared to cesium's 9.2 billion. More cycles per second means finer resolution, the way a ruler with millimeter marks is more precise than one with only centimeter marks.
...
In July 2024, a strontium clock at JILA; a research institute partnered with NIST in Boulder; achieved an accuracy of one second in 40 billion years. The universe is about 14 billion years old.

One year later, NIST's aluminum clock surpassed it. One second in 60 billion years.

The aluminum clock works differently. Instead of measuring billions of atoms at once, it measures one. A single aluminum atom, stripped of an electron; what physicists call an ion; and held in an electromagnetic trap. Aluminum's energy transition is ideal for timekeeping, but the ion moves too much at room temperature for a clean measurement. So a partner magnesium ion sits next to it. The magnesium handles cooling; the aluminum provides the tick. The two sit close enough that the magnesium can sense when the aluminum absorbs energy, letting physicists read the aluminum's transition through its partner. One atom, telling time.
...
In November 2025, the JILA group tried something different; not building a better clock, but linking the atoms inside one. In a normal atomic clock, each atom is measured on its own. The random noise of those independent measurements sets a floor on how precise any clock can get. The JILA team found a way to correlate the atoms; to make each measurement inform the others. The noise partially cancelled. The precision floor broke.

These clocks used to fill laboratories. They're leaving.

In January 2025, NIST packed a complete ytterbium optical lattice clock into shipping containers and sent it 3,000 kilometers; from Boulder to the U.S. Naval Observatory in Washington, DC. It was running within two days.
...
A better standard at the top matters only if the signal reaches your devices. NTP gets your laptop within a few milliseconds. But 5G base stations need nanoseconds.

A newer protocol; Precision Time Protocol, or PTP; is a thousand times tighter than NTP. In 2024, Meta open-sourced their entire time infrastructure; a simplified version of PTP that any organization can run on ordinary servers.
